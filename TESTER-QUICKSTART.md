# Pipeline Sentinel v0.13.0 — External Tester Quick Start

Pipeline Sentinel v0.13.0 is a **local workstation testing MVP**. It is intended to let an external tester install the packaged application, run ordinary video or image-sequence inputs, inspect the annotated output and interactive results, and report packaging/runtime problems.
...
- Internet access for the initial Python package install and first-run YOLO/DINOv2 model retrieval

Pipeline Sentinel itself is Apache-2.0 licensed. The optional Ultralytics YOLO and DINOv2 providers
and their model weights retain their own upstream license terms. See `LICENSE` and
...
The current production profile uses YOLO detection and DINOv2 representations, so external
acceptance testing should install the operator shell and both model extras:
...
The tester's imagery stays on their workstation; browser-selected files are copied only into the
local Pipeline Sentinel workspace, while Local Folder sources are read in place.
...
Leave **DINOv2 track representations** selected for the complete v0.13 test. Clear it only when
intentionally comparing against a YOLO/tracker-only run.
...
- interactive Detections, Tracks, Representations, Anomalies, Events / Alerts, and Downloads tabs;
...
The **Representations** tab should report sampled track embeddings. Representation change is
descriptive cosine-distance evidence and is not an anomaly score, event, or alert.
...
- `run_manifest.json` and `representation_manifest.json` for representation-stage problems
...
Do not send sensitive source imagery. Sanitize local paths, filenames, screenshots, and logs before
sharing them when necessary.
...
DINOv2 track representations are enabled in the production profile and may add first-run model
