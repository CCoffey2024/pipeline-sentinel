...
provide ordinary EO/IR media, run the packaged analytic pipeline, inspect job status, and retrieve
normal Pipeline Sentinel evidence artifacts without opening a notebook or editing source code.
...
6. Open the annotated-video and evidence artifacts.
...
## Test D — persistence and evidence
...
4. confirm completed artifacts are still accessible.
...
- whether the failure occurred during ingest, model loading, inference, rendering, or artifact review.
